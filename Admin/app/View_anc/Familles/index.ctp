	...
	<tr>
		...
		<td align="center">
			...
			<?php $articleS_edit=  CakeSession::read('articleS_edit'); 
                                if($articleS_edit==1){ 
                        echo $this->Html->link("<button class='btn btn-xs btn-warning'><i class='fa fa-edit'></i></button>", array('action' => 'edit', $famille['Famille']['id']),array('escape' => false));
                         } $articleS_delete=  CakeSession::read('articleS_delete'); 
                                if($articleS_delete==1){
                        echo $this->Form->postLink("<button class='btn btn-xs btn-danger'><i class='fa fa-trash-o'></i></button>", array('action' => 'delete', $famille['Famille']['id']),array('escape' => false,null), __('Veuillez vraiment supprimer cette enregistrement # %s?', $famille['Famille']['id'])); 
                                }?>
		</td>
	</tr>
<?php endforeach; ?>
