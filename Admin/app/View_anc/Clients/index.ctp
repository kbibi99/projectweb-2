<?php
$client_add=  CakeSession::read('client_add'); 
if($client_add==1){
?>
<div class="row">
    <div class="col-md-12">
...
<!--		<td >
		...
		<td align="center">
			...
			<?php $client_edit=  CakeSession::read('client_edit'); 
if                              ($client_edit==1){
                        echo $this->Html->link("<button class='btn btn-xs btn-warning'><i class='fa fa-edit'></i></button>", array('action' => 'edit', $client['Client']['id']),array('escape' => false)); 
                        }?>
			<?php $client_delete=  CakeSession::read('client_delete'); 
                            if($client_delete==1){
                        echo $this->Form->postLink("<button class='btn btn-xs btn-danger'><i class='fa fa-trash-o'></i></button>", array('action' => 'delete', $client['Client']['id']),array('escape' => false,null), __('Veuillez vraiment supprimer cette enregistrement # %s?', $client['Client']['id']));
                            }?>
		</td>
	</tr>
<?php endforeach; ?>
