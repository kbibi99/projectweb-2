...
<!--		<td >
		...
		<td align="center">
			...
			<?php echo $this->Html->link("<button class='btn btn-xs btn-success'><i class='fa fa-search'></i></button>", array('action' => 'view', $collection['Collection']['id']),array('escape' => false)); ?>
			   ...
			   <?php $collection_delete=  CakeSession::read('collection_delete'); 
                                if($collection_delete==1){
                           echo $this->Form->postLink("<button class='btn btn-xs btn-danger'><i class='fa fa-trash-o'></i></button>", array('action' => 'delete', $collection['Collection']['id']),array('escape' => false,null), __('Veuillez vraiment supprimer cette enregistrement # %s?', $collection['Collection']['id']));
                                }?>
		</td>
