<?php
$entreecaisse_add=  CakeSession::read('entreecaisse_add'); 
if($entreecaisse_add==1){
?>
<div class="row">
    <div class="col-md-12">
...
<div class="row">
	...
	<tr>
		...
		<td align="center">
			...
			<?php $entreecaisse_edit=  CakeSession::read('entreecaisse_edit'); 
                            if($entreecaisse_edit==1){
                        echo $this->Html->link("<button class='btn btn-xs btn-warning'><i class='fa fa-edit'></i></button>", array('action' => 'edit', $entreecaiss['Entreecaiss']['id']),array('escape' => false));
                            } ?>
			<?php  $entreecaisse_delete=  CakeSession::read('entreecaisse_delete'); 
                            if($entreecaisse_delete==1){
                        echo $this->Form->postLink("<button class='btn btn-xs btn-danger'><i class='fa fa-trash-o'></i></button>", array('action' => 'delete', $entreecaiss['Entreecaiss']['id']),array('escape' => false,null), __('Veuillez vraiment supprimer cette enregistrement # %s?', $entreecaiss['Entreecaiss']['id'])); 
                            }?>
		</td>
	</tr>
<?php endforeach; ?>
