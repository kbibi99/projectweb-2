...
$pdf->SetHeaderData('BLUE.png', '60', '                                                Liste Des Caisses Valides  ' );
...
$tbl .= '
   <tr bgcolor="#FFFFFF" align="center">    
        <td colspan="4" width="85%" nobr="nobr" align="center"   $zz></td>
             ...
             // if($Type="Cheque")
                {
                  ...
                  $tot_ch=0;$hhc=0;
                  foreach ($pieces_ch as $key => $value){ 
              ...
              if(!empty($pieces_tr))
                {
                  ...
                  $pdf->writeHTML($tbl, true, false, false, false, '');
                   ...
                   $tot_tr=0;$hht=0;
                  foreach ($pieces_tr as $key => $value){ 
                       $hht++;
                      if($hht==15){
                          $tbl.='</table>';
                          $pdf->writeHTML($tbl, true, false, false, false, '');
...
ob_end_clean();
$pdf->Output('Liste caisses Valides.pdf', 'I');
