...
<div class="row">
    <div class="col-md-12" >
        ...
        <?php echo $this->Form->create('Entreecaisses',array('autocomplete' => 'off','class'=>'form-horizontal ls_form','id'=>'defaultForm','data-bv-message'=>'This value is not valid','data-bv-feedbackicons-valid'=>'fa fa-check','data-bv-feedbackicons-invalid'=>'fa fa-bug','data-bv-feedbackicons-validating'=>'fa fa-refresh')); ?>
  ...
  <div class="col-md-6"> 
	...
	echo $this->Form->input('Type',array('div'=>'form-group','between'=>'<div class="col-sm-10">','after'=>'</div>','class'=>'form-control','required data-bv-notempty-message'=>'Champ Obligatoire','empty'=>'Veuillez choisir','id'=>'BoncommandeClientId') );
        ...
        ?>
                ...
                <div class="form-group">
                                            <div class="col-lg-9 col-lg-offset-3">
                                                ...
                                                <a href="<?php echo $this->webroot;?>Entreecaisses/liste_envoye" class="btn btn-primary">Afficher Tout</a>
                                                <a onClick="flvFPW1(wr+'Entreecaisses/imprimerliste_envoye?Type=<?php echo @$Type;?>&client_id=<?php echo @$client_id;?>&Date_debut=<?php echo @$Date_debut;?>&Date_fin=<?php echo @$Date_fin;?>','UPLOAD','width=800,height=1150,scrollbars=yes',0,2,2);return document.MM_returnValue" href="javascript:;" ><button class="btn btn-primary">Imprimer</button> </a>
<?php 
// debug($mar);
...
<?php echo $this->Form->end();?>
    ...
    <div class="col-md-12">
        <div class="panel panel-default">
            <div class="panel-heading">
                <h3 class="panel-title"><?php echo __('Les caisses envoyées'); ?></h3>
            </div>
	...
	<?php foreach ($pieces as $key => $value): 
            ...
            $value['Reglement']['Date']=date("d/m/Y",strtotime(str_replace('-','/',($value['Reglement']['Date']))));
            ?>
                            <tr>
                                <td><?php echo h($value['Piece']['Client']['Raison_Social']);?></td>
                                <td><?php echo h($value['Piece']['num_recu']);?></td>
                                <td><?php echo h($value['Reglement']['Date']);?></td>
                                <td><?php echo h($value['Paiement']['name']); ?> <?php if( ($value['Piecereglement']['paiement_id'] == 2)||($value['Piecereglement']['paiement_id'] == 3)){
                                   echo '(Numéro : '.$value['Piece']['num'].' | Echéance : '.$value['Piece']['echance'].' | Banque : '.$value['Piece']['banque'].')';
                                }
                                ?></td>
                                <td align="right"><?php echo h($value['Piecereglement']['Montant']); ?></td>
